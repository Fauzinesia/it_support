...
public function update($id)
{
    ...
    // Mengambil data progress dan deskripsi untuk update sistem tracking ticket
    ...
    $config['max_size']       = '20480';     // Ukuran maksimum file
    $config['max_width']      = '0';        // Tidak ada batas lebar
    $config['max_height']     = '0';        // Tidak ada batas tinggi

    // Memanggil library upload pada CodeIgniter dan menyimpan konfigurasi
    ...
    if (!empty($_FILES['fileupdate']['name'])) {
        ...
        if (!$this->upload->do_upload('fileupdate')) {
            ...
            // File berhasil diupload, ambil informasi file
            $gambar = $this->upload->data()['file_name'];
        }
    }

    // Menentukan status ticket berdasarkan progress
    if ($progress == 100) {
        $status = (strtotime($date) > strtotime($sql->deadline)) ? 7 : 6;
